...
class ADAPD:
    ...
    def __init__(self, local_params, mixing_matrix, training_data, init_weights):
        ...
        if 'model' in local_params:
            self.model_type = local_params['model']
        else:
        ...
        # We either have the Fully Connected arch or LENET
        if self.model_type == 'fc':
            self.model = FC(10).to(self.device)
        else:
        ...
        if self.model_type == 'fc':
            self.testing_model = FC(10).to(self.device)
        else:
            self.testing_model = LENET(10).to(self.device)
        ...
        # Initialize all of the variables; Y^0 = D^0
    ...
    def solve(self, outer_iterations, training_data_full_sample, testing_data):
        ...
        :param outer_iterations: number of COMMUNICATIONS to perform
        :param training_data_full_sample: data loader using the full batch of size_training_data
        :param testing_data: testing data loader with some portion of the testing data (size_testing_data // size)
        :return:
        '''
        ...
        for i in range(outer_iterations):
            ...
            self.Y = [self.Y[k] + (1 / self.eta) * (self.weights[k] - self.global_variables[k]) for k in
            ...
            self.Z = [self.Z[k] + (1 / self.eta) * self.neighbor_globals[k].to(self.device) for k in
                      range(self.num_params)]
            ...
            if i % self.report == 0:
                ...
                if rank == 0:
                    ...
                    print("{:<10} | {:<7} | {:<13} | {:<15} | {:<15} | {:<12} | {:<6}".format(i,
                                                round((i * self.mini_batch * self.local_max) / 7500, 2), round(total, 4),
...
if __name__=='__main__':
    ...
    # Parse user input
    parser = argparse.ArgumentParser(description='Testing ADAPD on MNIST dataset.')
    ...
    parser.add_argument('--model', type=str, default='lenet', choices=['lenet', 'fc'], help='Architecture to use.')
    parser.add_argument('--init', type=str, default='lenet', choices=['lenet', 'fc'], help='Initialization type.')
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.MNIST('data', train=True, download=True,
                         transform=transform),
        batch_size=args.mini_batch, sampler=torch.utils.data.SubsetRandomSampler([i for i in range(int(rank * num_samples), int((rank + 1) * num_samples))]))
    ...
    test_loader = torch.utils.data.DataLoader(
        ...
        batch_size=num_test, sampler=torch.utils.data.SubsetRandomSampler([i for i in range(int(rank * num_test), int((rank + 1) * num_test))]))
    ...
    model_length = 6 if args.model == 'fc' else 8
    init_weights = [
        numpy.load(os.path.join(os.getcwd(),
                                f'init_weights_{args.init}/trial{args.trial}/{args.model}/rank{rank}/layer{l}.dat'),
                   allow_pickle=True)
        for l in range(model_length)]

    # Print training information
    if rank == 0:
        if args.og:
            opening_statement = f' ADAPD-OG on MNIST '
        else:
            opening_statement = f' ADAPD on MNIST '
    ...
    # Declare and train!
    algo_params = {'model': args.model, 'local_lr': args.lr, 'mini_batch': args.mini_batch, 'eta': args.eta,
                   'local_max': args.local_max, 'report': args.report, 'local_solver': args.local_solver,
                   'l1': args.l1, 'og': args.og}
    ...
    # Model
    try:
        os.mkdir(os.path.join(os.getcwd(), f'results/{method}/trial{args.trial}/{args.model}'))
    except:
    ...
    # Initialization
    try:
        os.mkdir(os.path.join(os.getcwd(), f'results/{method}/trial{args.trial}/{args.model}/{args.init}'))
    except:
        # Model already exists
        pass
    ...
    try:
        os.mkdir(os.path.join(os.getcwd(),
                              f'results/{method}/trial{args.trial}/{args.model}/{args.init}/{args.comm_pattern}{size}'))
    except:
        # Model already exists
        pass
    ...
    try:
        os.mkdir(os.path.join(os.getcwd(),
                              f'results/{method}/trial{args.trial}/{args.model}/{args.init}/{args.comm_pattern}{size}/{args.mini_batch}'))
    except:
        # Model already exists
        pass

    # Save path
    path = f'results/{method}/trial{args.trial}/{args.model}/{args.init}/{args.comm_pattern}{size}/{args.mini_batch}'

    # Save information via numpy
    if rank == 0:
        numpy.savetxt(
            f'{path}/test_loss_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.testing_loss, fmt='%.7f')
        numpy.savetxt(
            f'{path}/test_acc_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.testing_accuracy, fmt='%.7f')
        numpy.savetxt(
            f'{path}/train_loss_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.training_loss, fmt='%.7f')
        numpy.savetxt(
            f'{path}/train_acc_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.training_accuracy, fmt='%.7f')
        numpy.savetxt(
            f'{path}/total_opt_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.total_optimality, fmt='%.7f')
        numpy.savetxt(
            f'{path}/consensus_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.consensus_violation, fmt='%.7f')
        numpy.savetxt(
            f'{path}/norm_hist_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.norm_hist, fmt='%.7f')
        numpy.savetxt(
            f'{path}/iterate_hist_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.iterate_norm_hist, fmt='%.7f')
        numpy.savetxt(
            f'{path}/total_time_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.total_time, fmt='%.7f')
        numpy.savetxt(
            f'{path}/comm_time_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
            solver.communication_time, fmt='%.7f')
        numpy.savetxt(
            f'{path}/comp_time_eta{args.eta}_solver{args.local_solver}_max{args.local_max}_stepsize{args.lr}_l1{args.l1}.txt',
